...
<form class="search-form form<?php echo $form_classes; ?>" <?php echo $aria_label; ?>role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>">
	...
	<?php } else { ?>
		<div id="search" class="navigation__search search">
			<label for="form-search-input" class="sr-only"><?php _ex('Поиск', 'label', 'bootstrap-basic'); ?></label>
			<input id="form-search-input"
				class="input input--search"
				type="search"
				name="s"
				value="<?php echo get_search_query(); ?>"
				placeholder="<?php echo esc_attr_x('Поиск по сайту…', 'placeholder', 'bootstrap-basic'); ?>"
				aria-label="<?php echo esc_attr_x('Поиск по сайту', 'aria label', 'bootstrap-basic'); ?>">
			<button class="search__button" type="submit" aria-label="<?php esc_attr_e('Выполнить поиск', 'bootstrap-basic'); ?>">
