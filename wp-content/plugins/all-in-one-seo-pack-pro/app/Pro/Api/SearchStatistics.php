...
class SearchStatistics {
	...
	public static function getInspectionResult( $request ) {
		...
		// Create an array with $path => false.
		$responseArray = array_map( '__return_false', array_flip( $paths ) );
		$quotaExceeded = ! empty( aioseo()->core->cache->get( 'search_statistics_url_inspection_quota_exceeded' ) );
		$missingResult = [];

		// First check if we have an object for these paths.
		$objects = aioseo()->core->db->start( 'aioseo_search_statistics_objects as asso' )
			->select( 'asso.id, asso.object_path, asso.inspection_result' )
		...
		foreach ( $paths as $path ) {
			// If we have an object, check if the inspection result is empty.
			if ( isset( $objects[ $path ] ) && ! empty( $objects[ $path ]->inspection_result ) ) {
				$responseArray[ $path ] = json_decode( $objects[ $path ]->inspection_result );
				continue;
			}

			// Otherwise we will need to fetch results for this path.
			$missingResult[] = $path;
		}

		if ( ! empty( $missingResult ) && ! $quotaExceeded ) {
			$api      = new Main\Api\Request( 'google-search-console/url-inspection/', [ 'paths' => $missingResult ], 'GET' );
			$response = $api->request();
