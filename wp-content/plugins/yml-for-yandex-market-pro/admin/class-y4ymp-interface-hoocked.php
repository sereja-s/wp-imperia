...
final class Y4YMP_Interface_Hoocked {
				...
				<td>
					...
					<p class="description">
						<?php esc_html_e( 'Category for Yandex Market', 'yml-for-yandex-market-pro' ); ?>. <a
							href="https://icopydoc.ru/para-sovetov-po-ispolzovaniyu-yml-for-yandex-market-pro/?utm_source=yml-for-yandex-market-pro&utm_medium=organic&utm_campaign=in-plugin-yml-for-yandex-market-pro&utm_content=edit-category&utm_term=about-category-ymarket"
							target="_blank">
	...
	public function save_meta_product_cat( $term_id ) {
		...
		if ( isset( $_POST['yfymp_cat_meta'] ) ) {
			if ( isset( $_POST['yfymp_disabled_for_feeds'] ) ) {
				$disabled_for_feeds = $_POST['yfymp_disabled_for_feeds']; // TODO: дезинфекция массива
			} else {
				$disabled_for_feeds = '';
			}
	...
	public function bulk_action_handler( $redirect_to, $doaction, $post_ids ) {
		...
		if ( $doaction == 'add_yml_kit' ) {
			...
			foreach ( $post_ids as $post_id ) {
				update_post_meta( $post_id, 'yfymp_removefromxml', '' );
			}
		} else {
		...
		<div class="options_group">
			...
			woocommerce_wp_select( [ 
				...
				'description' => sprintf( '<a target="_blank" href="%s/?%s">%s</a>',
					...
					'utm_source=yml-for-yandex-market-pro&utm_medium=organic&utm_campaign=in-plugin-yml-for-yandex-market-pro&utm_content=add_product&utm_term=about-yml-nabor',
		...
		if ( isset( $_POST['_y4ymp_full_edit'] ) ) {
			if ( isset( $_POST['yfymp_removefromthisyml_arr'] ) ) {
				update_post_meta(
					...
					$_POST['yfymp_removefromthisyml_arr'] // TODO: Дезинфекция
				);
			} else {
				update_post_meta(
					$post_id,
