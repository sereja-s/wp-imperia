...
final class Y4YMP_Generation_Hoocked {
	...
	public function init_hooks() {
		...
		add_filter( 'y4ym_f_simple_skip_vendor_reason', [ $this, 'simple_skip_vendor_reason' ], 10, 3 );
		add_filter( 'y4ym_f_variable_skip_vendor_reason', [ $this, 'variable_skip_vendor_reason' ], 10, 3 );
	...
	public function change_stop_flag( $stop_flag, $data_arr, $feed_id ) {
		...
		if ( $one_variable === 'enabled' || $one_variable === 'on' ) { // TODO: Удалить 'on'
			return true;
		} else {
			return $stop_flag;
		}
	...
	public function change_skip_flag( $skip_flag, $data_arr, $feed_id ) {
		...
		if ( $whot_export === 'vygruzhat' ) {
			if (
				get_post_meta( $product->get_id(), 'vygruzhat', true ) === 'enabled'
				// TODO: удалить в след.версиях этот вариант
				|| get_post_meta( $product->get_id(), 'vygruzhat', true ) === 'on'
			) {
	 ...
	 * Filtering by brand.
	 * 
	 * Function for `y4ym_f_simple_skip_vendor_reason` action-hook.
	 * 
	 * @param mixed $skip_vendor_reason
	 * @param array $dara_arr keys: `product`, `vendor_name`.
	 * @param string $feed_id
	 * 
	 * @return mixed
	 */
	public function simple_skip_vendor_reason( $skip_vendor_reason, $dara_arr, $feed_id ) {
	...
	/** // TODO: В теории можно объёдинить с предыдущей функцией.
	 * Filtering by brand.
	 * 
	 * Function for `y4ym_f_variable_skip_vendor_reason` action-hook.
	 * 
	 * @param mixed $skip_vendor_reason
	 * @param array $dara_arr keys: `product`, `offer`, `vendor_name`.
	 * @param string $feed_id
	 * 
	 * @return mixed
	 */
	public function variable_skip_vendor_reason( $skip_vendor_reason, $dara_arr, $feed_id ) {
	...
	public function add_pic_simple_offer( $result_xml, $data_arr, $feed_id ) {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			// TODO: Возможно сюда нужно внедить пропуск default.png
		...
		// TODO: Возможно сюда нужно внедить фильтр
		return $result_xml;
	...
	/**
	 * Change simple price. // TODO: Заменить умножение цены на проценты
	 * 
	...
	/**
	 * Change variable price. // TODO: Заменить умножение цены на проценты
	 * 
	...
	public function change_simple_url( $tag_value, $data_arr, $feed_id ) {
		...
		if ( $use_utm === 'enabled' ) {
			$tag_value = sprintf(
				'%s?utm_source=%s&amp;utm_campaign=%s&amp;utm_content=%s&amp;utm_medium=%s&amp;utm_term=%s',
				$tag_value,
	...
	public function append_variable_offer( $result_xml, $args_arr, $feed_id ) {
		...
		if ( ! empty( $constructor_params_arr ) && isset( $constructor_params_arr[ $feed_id ] ) ) {
			for ( $i = 1; $i < Y4YMP_PARAM_N; $i++ ) {
				...
				if ( $constructor_params_arr[ $i ]['param_use'] === 'enabled'
					|| $constructor_params_arr[ $i ]['param_use'] === 'on' // TODO: Удалить в след.версиях
				) {
					$tag_value = '';
					$attr_param_arr = []; // атрибуты параметра
					// name
