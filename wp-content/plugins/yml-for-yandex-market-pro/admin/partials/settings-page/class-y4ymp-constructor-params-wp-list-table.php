			...
			$result_arr[] = [ 
				'y4ymp_constructor_param_switcher' => sprintf( '%s <br/>Param %s: %s',
					...
					$this->get_select_html_v2(
						'param_use' . $i, 
						$constructor_params_arr[ $i ][ 'param_use' ], 
						$this->get_feed_id(),
						[ 
				...
				'y4ymp_constructor_param_name' => sprintf( '%s<br />%s<br />%s',
					$this->get_select_html_v2(
						'param_name_select' . $i, 
						$constructor_params_arr[ $i ][ 'param_name_select' ],
						$this->get_feed_id(),
						[ 
							'Размер' => 'Размер',
					...
					$this->get_input_html(
						'param_name_custom' . $i,
						$constructor_params_arr[ $i ][ 'param_name_custom'],
						$this->get_feed_id(),
						'type2'
					)
				),
				'y4ymp_constructor_param_unit' => sprintf( '%s<br />%s:<br />%s<br />%s:<br />%s',
					$this->get_select_html(
						'param_unit_select' . $i,
						$constructor_params_arr[ $i ][ 'param_unit_select' ],
						$this->get_feed_id(),
						[]
					),
					...
					$this->get_select_html_v2(
						...
						$constructor_params_arr[ $i ][ 'param_unit_default_select' ],
						$this->get_feed_id(),
						[ 
							'AU' => 'AU',
							'DE' => 'DE',
					...
					$this->get_input_html(
						'param_unit_custom' . $i,
						$constructor_params_arr[ $i ][ 'param_unit_custom' ],
						$this->get_feed_id(),
						'type2'
					)
				),
				'y4ymp_constructor_param_value' => sprintf( '%s<br />%s:<br />%s',
					$this->get_select_html(
						...
						$constructor_params_arr[ $i ][ 'param_value_select' ],
						$this->get_feed_id(),
						[]
					),
					__( 'or specify custom value', 'yml-for-yandex-market-pro' ),
					$this->get_input_html(
						...
						$constructor_params_arr[ $i ][ 'param_value_custom' ],
						$this->get_feed_id(),
						'type2'
					)
				)
	...
	private function get_select_html( $opt_name, $opt_value, $feed_id = '1', $otions_arr = [] ) {
		...
		if ( isset( $otions_arr['brands'] ) ) {
			...
			if ( is_plugin_active( 'woocommerce-brands/woocommerce-brands.php' ) ) {
				$res .= '<option value="woocommerce_brands" ' . selected( $opt_value, 'woocommerce_brands', false ) . '>' . __( 'Substitute from', 'yml-for-yandex-market-pro' ) . 'WooCommerce Brands</option>';
			}
