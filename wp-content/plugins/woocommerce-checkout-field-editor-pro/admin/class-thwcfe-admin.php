...
class THWCFE_Admin {
	...
	public function enqueue_styles_and_scripts($hook) {
		...
		}else if($screen_id === 'shop_order' && apply_filters('thwcfe_edit_order_custom_checkout_fields', false)){
			
			$debug_mode = apply_filters('thwcfe_debug_mode', false);
			$suffix = $debug_mode ? '' : '.min';
			$jquery_version = isset($wp_scripts->registered['jquery-ui-core']->ver) ? $wp_scripts->registered['jquery-ui-core']->ver : '1.9.2';

			$this->enqueue_orderdetails_styles($suffix, $jquery_version);
			$this->enqueue_orderdetails_scripts($suffix, $jquery_version);
		}
	}
	...
	private function enqueue_orderdetails_styles($suffix, $jquery_version, $in_footer=true) {
		wp_enqueue_style('thwcfe-public-myaccount-style', THWCFE_ASSETS_URL_PUBLIC . 'css/thwcfe-public'. $suffix .'.css', THWCFE_VERSION );
	}

	private function enqueue_orderdetails_scripts($suffix, $jquery_version, $in_footer=true) {
		wp_enqueue_script('thwcfe-orderdetails-script', THWCFE_ASSETS_URL_PUBLIC.'js/thwcfe-orderdetails'. $suffix .'.js', array(), THWCFE_VERSION, $in_footer);
			
		$wcfe_var = array(
			'ajax_url'    => admin_url( 'admin-ajax.php' ),
		);
		wp_localize_script('thwcfe-orderdetails-script', 'thwcfe_public_var', $wcfe_var);
	}
	
	public function admin_menu() {
		$capability = THWCFE_Utils::wcfe_capability();
		add_submenu_page('woocommerce', __('WooCommerce Checkout Field Editor Pro', 'woocommerce-checkout-field-editor-pro'), __('Checkout Form','woocommerce-checkout-field-editor-pro'), $capability, 'th_checkout_field_editor_pro', array($this, 'output_settings'));

		// Add setup wizard page admin access.
		add_dashboard_page( '', '', 'manage_options', 'thwcfe-welcome', );
	}
	...
	public function plugin_action_links($links) {
		$settings_link = '<a href="'.admin_url('admin.php?page=th_checkout_field_editor_pro').'">'. __('Settings', 'woocommerce') .'</a>';
	...
	public function plugin_row_meta( $links, $file ) {
		if(THWCFE_BASE_NAME == $file) {
			...
			$row_meta = array(
				'docs' => '<a href="'.$doc_link.'" target="_blank" aria-label="'.__('View plugin documentation','woocommerce-checkout-field-editor-pro').'">'.__('Docs','woocommerce').'</a>',
				'support' => '<a href="'.$support_link.'" target="_blank" aria-label="'. __('Visit premium customer support','woocommerce-checkout-field-editor-pro' ) .'">'. __('Premium support', 'woocommerce') .'</a>',
			);
	...
	public function output_settings(){
		...
		if($tab === 'advanced_settings'){			
			...
			if($c_type === 'block'){
				$block_settings = THWCFE_Admin_Settings_Block::instance();	
				$block_settings->init();
			}else{
		...
		<div id="th_block_warning" class="th-block-warning-msg">
            <div class="th-warning-message-panel__text th-warning-message-panel__text--center">
				...
				<div class="th-warning">
					...
					<?php
						esc_html_e('Our Checkout Field Editor now supports Checkout Blocks! This initial release has fewer features than the Classic Editor, but more powerful updates are coming soon—stay tuned!', 'woocommerce-checkout-field-editor-pro'); 
					?>
					</span>
				</div>
            </div>
	...
	public function print_js_variables(){
		...
		$tab_type = isset($_GET['c_type']) ? sanitize_text_field($_GET['c_type']) : 'classic';
		$content = "";
		$checkout_fields = array();
		//$sections = THWCFE_Utils::get_custom_sections();
		$sections = ( $tab_type === 'block' )
     		? THWCFE_Utils_Block::get_block_checkout_sections()
     		: THWCFE_Utils::get_custom_sections();
	...
	public function fb_collection_actions(){
		if( !( isset($_GET['thwcfe_fb_dismiss']) || isset($_GET['thwcfe_fb_added'])) ) {
			return;
		}

		$nonse = isset($_GET['thwcfe_fb_nonce']) ? sanitize_text_field( wp_unslash($_GET['thwcfe_fb_nonce'])) : false;
		$capability = THWCFE_Utils::wcfe_capability();

		if(!wp_verify_nonce($nonse, 'thwcfe_fb_security') || !current_user_can($capability)){
			die();
		}

		$thwcfe_dismiss = isset($_GET['thwcfe_fb_dismiss']) ? sanitize_text_field( wp_unslash($_GET['thwcfe_fb_dismiss'])) : false;
		if($thwcfe_dismiss){
			update_user_meta( get_current_user_id(), 'thwcfe_fb_dismissed', true );
		}

		$thwcfe_fb_added = isset($_GET['thwcfe_fb_added']) ? sanitize_text_field( wp_unslash($_GET['thwcfe_fb_added'])) : false;
		if($thwcfe_fb_added){
			update_user_meta( get_current_user_id(), 'thwcfe_fb_added', true );
		}
	}

	public function fb_collection_links(){
		$current_screen = get_current_screen();
		if($current_screen->id !== 'woocommerce_page_th_checkout_field_editor_pro'){
			return;
		}

		if(get_user_meta( get_current_user_id(), 'thwcfe_fb_dismissed', true ) || get_user_meta( get_current_user_id(), 'thwcfe_fb_added', true )){
			return;
		}

		$dismiss_url = add_query_arg(array(
			'thwcfe_fb_dismiss' => true, 
			'thwcfe_fb_nonce' => wp_create_nonce( 'thwcfe_fb_security'
		)), admin_url('admin.php?page=th_checkout_field_editor_pro'));

		?>
		<div class="th_feedback_widget">
			<form action="" method="POST"  id="th_dashboard_feedback" >
				<a class="close" href="<?php echo esc_url($dismiss_url); ?>">
					<svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M1 1.25786C1.00401 1.25176 1.00744 1.24529 1.01024 1.23853C1.08211 0.993028 1.35531 0.919956 1.53673 1.10012C1.86493 1.42605 2.19216 1.75313 2.51843 2.08137L3.95107 3.51439C3.96595 3.52947 3.9816 3.54378 4.01406 3.57471C4.02464 3.55274 4.0376 3.53199 4.0527 3.51285C4.84988 2.71382 5.64738 1.91551 6.4452 1.11791C6.53582 1.02724 6.63706 0.978916 6.76632 1.01139C6.81535 1.02341 6.86066 1.04735 6.89822 1.08109C6.93579 1.11484 6.96444 1.15734 6.98165 1.20483C6.99885 1.25231 7.00407 1.30332 6.99684 1.35331C6.98961 1.4033 6.97016 1.45073 6.9402 1.49139C6.92103 1.51599 6.90004 1.53912 6.87741 1.56059C6.08049 2.35691 5.28376 3.15329 4.48723 3.94973C4.46465 3.96936 4.44437 3.99147 4.42675 4.01565C4.4484 4.02488 4.46879 4.03683 4.48742 4.05122C5.28717 4.85024 6.08661 5.64927 6.88572 6.44829C6.95508 6.51749 7.0001 6.59501 6.99836 6.69592C6.9976 6.7567 6.97884 6.81588 6.94444 6.86599C6.91005 6.91609 6.86158 6.95486 6.80515 6.97738C6.78757 6.98434 6.77037 6.99227 6.75279 6.99961H6.63571C6.54702 6.97371 6.48114 6.91688 6.41584 6.85231C5.62845 6.06424 4.8408 5.27604 4.05289 4.48772C4.0382 4.46942 4.02575 4.44943 4.0158 4.42818L3.98759 4.43282C3.97559 4.45322 3.96196 4.47262 3.94682 4.49081C3.16072 5.27708 2.37481 6.06366 1.58909 6.85057C1.5234 6.91649 1.45578 6.97564 1.36381 7H1.24653C1.18653 6.98245 1.1322 6.94939 1.08903 6.90415C1.04585 6.85891 1.01534 6.8031 1.00058 6.74231V6.63677C1.02879 6.54011 1.09409 6.46975 1.16365 6.40035C1.94576 5.61975 2.72722 4.83871 3.50804 4.05721C3.52747 4.04089 3.54836 4.02639 3.57045 4.01391V3.98549C3.54826 3.97322 3.5273 3.95885 3.50785 3.94258C2.72613 3.16121 1.94486 2.38023 1.16403 1.59964C1.09448 1.53024 1.02879 1.46046 1.00058 1.36341L1 1.25786Z" fill="black"
						stroke="black"
						stroke-width="0.5"
						/>
					</svg>
				</a>
				<div class="th-fb-content">
					<div id="4869afec-3b76-4a8d-a2b7-3aea2cb61059" class="th-feedback-icons" data-nonce="<?php echo wp_create_nonce('thwcfe_fb_security'); ?>">
						<label><?php esc_html_e('Are you finding it easy to differentiate between Classic and Block Checkout Editors?','woocommerce-checkout-field-editor-pro' ); ?></label>
						<input name="thwcfe_db_fb" type="radio" value="yes" id="yes" >
						<label key="yes" for="yes">
							<svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M4.72565 14.7481V7.80592H1V16.1365H4.72565V14.4704C5.65706 15.5811 7.27151 16.1365 8.57549 16.1365H12.6116C13.2947 16.1365 13.9156 15.5811 14.0398 14.7481L14.9712 8.77782C15.1575 7.7365 14.4123 7.1117 13.543 7.1117H10.3141V2.52989C10.3141 1.62741 9.5069 0.86377 8.69968 0.86377C7.89246 0.86377 7.20942 1.62741 7.20942 2.52989C7.20942 3.64063 6.89895 4.68195 6.3401 5.58443L4.72565 7.66708" stroke="#1C274C" stroke-width="1.3" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							</svg>
						</label>
						<input type="radio" id="no" name="thwcfe_db_fb" value="no">
						<label for="no" key="no">
							<svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path d="M11.2743 2.25193L11.2743 9.19408L15 9.19408L15 0.863502L11.2743 0.863501L11.2743 2.52962C10.3429 1.41887 8.72849 0.863501 7.42451 0.863501L3.38839 0.863501C2.70535 0.863501 2.08441 1.41887 1.96022 2.25193L1.02881 8.22218C0.842524 9.2635 1.58765 9.88829 2.45697 9.88829L5.68587 9.8883L5.68587 14.4701C5.68587 15.3726 6.4931 16.1362 7.30032 16.1362C8.10754 16.1362 8.79058 15.3726 8.79058 14.4701C8.79058 13.3594 9.10105 12.318 9.6599 11.4156L11.2743 9.33292" stroke="#1C274C" stroke-width="1.3" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							</svg>
						</label>
						
					</div >
				</div>
				<div class = "th-fb-submit" style="display: none;">
					<input id="61d27d90-4699-4f1a-8ba4-7a02b91961e1" placeholder="Tell us more..."  name="thwcfe_fb_note" type="text" />
					<!-- <input type="submit" value="Submit" /> -->
					<button class="thwcfe-fb-submit" type="button" value="Submit"> Submit </button>
				</div>
			</form>
		</div>
		<?php
	...
	public function quick_links(){
		...
		<div class="th_quick_widget-float">
			<div id="myDIV" class="th_quick_widget">
				<div class="th_whead">
					<div class="th_whead_close_btn" onclick="thwcfewidgetClose()">
						<svg
						width="8"
                		height="8"
                		viewBox="0 0 8 8"
...
<!--                			 	<li>
                    			...
                    			<a href="https://www.themehigh.com/product/woocommerce-checkout-field-editor-pro/?utm_source=free&utm_medium=premium_tab&utm_campaign=wcfe_upgrade_link" target="_blank" class="quick-widget-doc-link">Upgrade to Premium</a></li> -->
               			 	<li>
				          	...
				          	<li>
		              			...
		              			</div><a href="https://www.themehigh.com/docs/support/" target="_blank" class="quick-widget-support-link">Get support</a></li>
		              		<li>
			...
			<div id="myWidget" class="widget-popup" onclick="thwcfewidgetPopUp()">
	...
	public function redirect_to_landing_page() {
		...
		wp_safe_redirect( esc_url_raw( admin_url( 'index.php?page=thwcfe-welcome' ) ) );
		exit();
	}

	public function landing_page(){
		if ( empty( $_GET['page'] ) || 'thwcfe-welcome' !== $_GET['page'] ) {
			return;
		}
		add_filter('deprecated_function_trigger_error', '__return_false');
		$this->enqueue_landing_page_styles();
		ob_start();
		include_once THWCFE_ABSPATH . '/admin/thwcfe-landing-page.php';
	}

	public function enqueue_landing_page_styles() {
		wp_enqueue_style( 'thwcfe-landing-page', THWCFE_ASSETS_URL_ADMIN . 'css/landing-page.min.css', array(), THWCFE_VERSION );
	}

	private function is_existing_user() {

		$thwcfe_user = get_option( 'thwcfe_user', false );
		$thwcfe_since = get_option('thwcfe_since', false);
    	$now = time();
		if($thwcfe_user === 'new_user'){
			update_option( 'thwcfe_since', $now, 'no' );
			update_option( 'thwcfe_user', 'existing_user', 'no' );
			return false;
		}
