			...
			if(!empty($value) || (($value === '0') && apply_filters( 'thwcfe_accept_value_zero',false))){
				...
				if($type === 'file'){
					...
					if($type === 'datepicker'){
						...
						$format = $field->get_property('date_format')? $field->get_property('date_format') : 'yyy-MM-dd';
						$dateString = $value;
						$formatMap = [
							'MM' => 'm',    // Month (2 digits)
							'dd' => 'd',    // Day (2 digits)
							...
							'yy' => 'y',    // Year (2 digits)
							'MMMM' => 'F',  // Month (full name)
							'MMM' => 'M',   // Month (short name)
							'EEE' => 'D',   // Day of the week (short name)
						];
						// Convert custom format to PHP format
						$phpFormat = strtr($format, $formatMap);

    					$date = new DateTime($dateString, new DateTimeZone('UTC'));
        				$formattedDate = $date->format($phpFormat);
        				$value = $formattedDate;
					}
